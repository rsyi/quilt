...
class PackageEntry(object):
    ...
    def __init__(self, physical_keys, size, hash_obj, meta):
        ...
        assert isinstance(physical_keys[0], PhysicalKey)
    ...
    def as_dict(self):
        ...
        """
        return {
            'physical_keys': [str(pk) for pk in self.physical_keys],
            'size': self.size,
            'hash': self.hash,
            'meta': self._meta
        }
    ...
    def get_cached_path(self):
        ...
        """
        physical_key = self.get()
        if not physical_key.is_local():
            return ObjectPathCache.get(str(physical_key))
        return None
    ...
    def deserialize(self, func=None, **format_opts):
        ...
        pkey_ext = pathlib.PurePosixPath(physical_key.path).suffix
...
class Package(object):
        ...
        def _create_str(results_dict, level=0, parent=True):
            ...
            if parent:
                has_remote_entries = any(
                    self._map(
                        lambda lk, entry: not entry.get().is_local()
                    )
                )
    ...
    def resolve_hash(cls, registry, hash_prefix):
        ...
        if len(hash_prefix) == 64:
            ...
            all_hashes = list_url(registry_parsed.join('.quilt/packages/'))
    ...
    def _load(cls, readable_file):
        ...
        for obj in reader:
            ...
            subpkg._children[key] = PackageEntry(
                [PhysicalKey.from_url(pk) for pk in obj['physical_keys']],
                obj['size'],
                obj['hash'],
                obj['meta']
            )
    ...
    def _fix_sha256(self):
        ...
        for entry in entries:
            physical_keys.append(entry.get())
    ...
    def _set(self, logical_key, entry=None, meta=None, serialization_location=None, serialization_format_opts=None):
        ...
        if isinstance(entry, (str, os.PathLike)):
            ...
            write_pk = PhysicalKey(None, str(serialization_path), None)
    ...
    def top_hash(self):
        ...
        for logical_key, entry in self.walk():
            if entry.hash is None or entry.size is None:
                raise QuiltException(
                    "PackageEntry missing hash and/or size: %s" % entry.get()
                )
    ...
    def push(self, name, registry=None, dest=None, message=None, selector_fn=lambda logical_key, package_entry: True):
        ...
        temp_file_logical_keys = [lk for lk, entry in self.walk() if physical_key_is_temp_file(entry.get())]
